...
$sinif_sor = $VeritabaniBaglantisi->prepare("SELECT sinif_adi FROM siniflar WHERE sinif_id=? "); //gelen sinifID'nin adını çektik
$sinif_sor->execute(array($GelenID));
$sinif_adi=$sinif_sor->fetch(PDO::FETCH_ASSOC);
            ?>
...
<body id="body-pd">
    ...
    <div class="col-8" style="margin-top: 80px;">
        <h4><?php echo $sinif_adi['sinif_adi'] . " adlı sınıfta staj formu onaylanan öğrencilerin listesi"  ?></h4>
    </div>
        <div class="col-4 mb-2" style="margin-top: 80px;"><button onclick='window.print("#myprint")' class="btn btn-dark">Yazdır</button> </div>
    ...
    <div style="border-top:2px solid blue; padding: 20px;">
        <table class="table table-dark table-striped">
            ...
            <?php
            $ogrenci_sor = $VeritabaniBaglantisi->prepare("SELECT * FROM ogrenciler WHERE Ogrenci_sinif_id=? and StajOnay=? and StajOnayBaskan=? ORDER BY Ogrenci_id ASC");
            $ogrenci_sor->execute(array($GelenID,1,1));
